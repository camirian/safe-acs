...
sequenceDiagram
    ...
    Sim->>Edge: Emit Telemetry (100Hz)
    Edge->>Edge: Check SysML Bounds (e.g., RPM < 6000)
    
    alt Telemetry Invalid (Fatal)
        Edge-->>Sim: Immediate Safe-Mode PID Override
        Edge->>Log: Record Constraint Violation
    else Telemetry Valid
        Edge->>Router: Pass State to AI Router
    end
    
    Router->>Claude: Heuristic Context Request
    Claude-->>Router: Proposed Control Action
    
    Router->>Edge: Re-verify Action against Bounds
    
    alt Action out-of-bounds
        Edge-->>Router: Reject Action
        Router->>Log: Record LLM Hallucination/Violation
    else Action within bounds
        alt Type 2 (Reversible / Software)
            Router->>Sim: Autonomous Actuation
            Router->>Log: Record Type 2 Success
        else Type 1 (Irreversible / Hardware)
            Router->>Human: Request Cryptographic Approval
            Human-->>Router: Approve Action
            Router->>Sim: Verified Actuation
            Router->>Log: Record Type 1 Human-in-Loop Success
        end
    end
```
> **Interpretation (Sequence Protocol):**
> Notice the "box" encompassing the Jetson Edge Node. **Claude cannot talk directly to the hardware.** Every heuristic thought generated by the LLM is intercepted by the `Decision Router` and passed back through the `Edge Guardrails` (Step 7) before actuation. If Claude hallucinates a fatal command, it is caught at Step 8 and logged, while the hardware remains completely isolated.
